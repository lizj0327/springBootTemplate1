...
public class ImageUtils {
    ...
    public static void main(String[] args) {
        // 1-缩放图像：
        // 方法一：按比例缩放
        ImageUtils.scale("e:/abc.jpg", "e:/abc_scale.jpg", 2.1f);// 测试OK
        // 方法二：按高度和宽度缩放
    ...
    public final static void scale(String srcImageFile, String result, float scale) {
        try {
            ...
            // 计算新的宽度和高度
            width = Integer.valueOf(Float.valueOf(width * scale).intValue());
            height = Integer.valueOf(Float.valueOf(height * scale).intValue());
    ...
    public final static void scale2(String srcImageFile, String result, int height, int width, boolean bb) {
        try {
            ...
            ImageIO.write(toBufferedImage(itemp), "JPEG", new File(result));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * @param width 期望最大宽度
     */
    public final static void decrease(String srcImageFile, String result, int height, int width) {
        try {
            // 读入文件
            BufferedImage src = ImageIO.read(new File(srcImageFile));
            // 得到源图宽
            ...
            if ((srcWidth > srcHeight) && ((srcWidth > width) || (srcHeight > height))) {
                ...
                ImageUtils.scale(srcImageFile, result, scale);
            }
            ...
            // 其他情况，按缩小倍数为1处理
                ...
                ImageUtils.scale(srcImageFile, result, 1f);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    /**
     * 判断文件是否为图片，以图片后缀名为依据<br>
     * <br>
     * 
     * @param pInput 文件名<br>
     * @param pImgeFlag 判断具体文件类型。 pImgeFlag为空的场合，表示验证全部后缀名，比如为“1”的场合表示判断后缀名是否为"bmp" <br>
     * @return 检查后的结果<br>
     * @throws Exception
     */
    public final static boolean isImage(String pInput, String pImgeFlag) {
        ...
        // 遍历名称数组
        for (String[] element : imgeArray) {

            if (StringUtils.isBlank(pImgeFlag)) {
                // 判断符合全部类型的场合
                if (element[0].equals(tmpName.toLowerCase())) {
                    return true;
                }
            } else {
                // 判断单个类型文件的场合
                if (element[0].equals(tmpName.toLowerCase()) && element[1].equals(pImgeFlag)) {
                    return true;
                }
            }
